...
def ask_alex_system(instruction: str) -> str:
    """
    Вызов на 8b — для коротких системных задач:
    будильники, фото-задания, реакции, подтверждения.
...
def ask_alex_smart(instruction: str) -> str:
    """
    Вызов на 70b — для задач где нужна глубина:
    брифинг, вечерний ритуал, разбор ситуации, генерация фразы для картинки.
    Не сохраняет в историю диалога.
    """
    return ask_alex(
        user_message=instruction,
